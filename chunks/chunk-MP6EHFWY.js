...
`},{path:"requirements/Requirements.sysml",text:`package MissionRequirements {
    ...
    part def MissionEvidence {
        attribute dryMass : MassValue;
        attribute powerMargin : PowerValue;
        attribute imagingDuration : TimeValue;
        attribute windSpeed : SpeedValue;
    }

    // Concrete evidence used by the checks below.
    part nominalMission : MissionEvidence {
        attribute dryMass : MassValue = 88 [kg];
        attribute powerMargin : PowerValue = 240 [W];
        attribute imagingDuration : TimeValue = 300 [s];
        attribute windSpeed : SpeedValue = 0 [m/s];
    }

    part overweightMission : MissionEvidence {
        attribute dryMass : MassValue = 108 [kg];
        attribute powerMargin : PowerValue = 240 [W];
    }

    part incompleteMission : MissionEvidence;

    requirement missionReadiness {
        subject mission : MissionEvidence;
...
`},{path:"views/SatelliteViews.sysml",text:`package SatelliteViews {
    // \`Views::*\` supplies the standard tree, interconnection, textual-notation,
    // and element-table renderings used below.
    ...
    rendering TextTable :> asElementTable {
        doc /* # Text table rendering

        Gives each exposed element one column containing canonical SysML
        textual notation.
        */
        view notation :>> columnView[1] {
            render asTextualNotation;
        }
    }

    rendering InspectionTable :> asElementTable {
        doc /* # Inspection table rendering

        Combines textual notation with nested structural and interconnection
        diagrams for each exposed element.
        */
        view notation :>> columnView[1] {
            render asTextualNotation;
        }
        view structure :>> columnView[1] {
            render asTreeDiagram;
        }
        view connections :>> columnView[1] {
            render asInterconnectionDiagram;
        }
    }

    rendering SafetyTable :> asElementTable {
        doc /* # Conditional table rendering

        Shows notation for every row and fills the second column only when the
        row carries \`@FlightCritical\` metadata.
        */
        view notation :>> columnView[1] {
            render asTextualNotation;
        }
        view flightCritical :>> columnView[1] {
    ...
    view subsystemInventory {
        doc /* # Subsystem inventory table

        Lists the observatory's major subsystem usages in an explicit row order
        with canonical textual notation in the \`notation\` column.
        */
        render TextTable;
        expose Satellite::observatory::panels;
        expose Satellite::observatory::batteries;
        expose Satellite::observatory::thrusters;
        expose Satellite::observatory::controller;
        expose Satellite::observatory::bus;
        expose Satellite::observatory::payload;
    }

    view subsystemInspection {
        doc /* # Nested subsystem inspection table

        Demonstrates non-textual cells by embedding tree and interconnection
        views beside the notation for the bus and payload usages.
        */
        render InspectionTable;
        expose Satellite::observatory::bus;
        expose Satellite::observatory::payload;
    }

    view segmentSafetyTable {
        doc /* # Conditional safety table

        The \`flightCritical\` column is blank for rows without the corresponding
        metadata, while every row retains its textual-notation column.
        */
        render SafetyTable;
        expose segment::solarBoom;
        expose segment::antennaBoom;
        expose segment::sunSensor;
        expose segment::starTracker;
        expose segment::testPort;
    }
...
`}]});export{b as a,g as b,f as c,y as d,h as e,S as f};
